...
def simulate_minute_consumption():
   try:
       ...
       cur.execute("""
           ...
           WHERE is_active = true AND id IN (9, 99, 999, 9999, 99999)
       """)
       ...
       for (house_id, risk_level, appliance_id, power, standby_power, app_type, is_active, 
           ...
           if app_type == 'CONSTANT':
               ...
               # SCHEDULED domy se budou implementovat později
               # Zde by přišla kontrola is_active v časových oknech pro EXTREME domy

           elif app_type == 'ON_DEMAND':
               # ON_DEMAND domy se budou implementovat později
       ...
       if 9 in houses and len(special_house_ids) > 1:
           ...
           for house_id in [h for h in special_house_ids if h != 9]:
               ...
               houses[house_id] = {
                   ...
                   'risk_level': houses[house_id]['risk_level']  # Ponecháme původní rizikový profil
               }
